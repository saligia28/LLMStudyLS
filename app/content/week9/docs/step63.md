...
这一节不只讲概念，而是要把 **整理 demo 数据** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 整理课程内容为 demo 数据集，完成一次端到端检索。

---

## 一、先把问题看清楚

如果只看文字解释，**整理 demo 数据** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week9/code/demo-data
```

这一节建议至少准备这些文件：

```
week9/code/demo-data/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week9/code/demo-data/server-demo.js
```
...
```js
// app/server/services/demo-data.service.js
export async function handleDemoData(payload) {
  return {
    ok: true,
    feature: 'demo-data',
    payload,
...
```js
// app/server/routes/demo-data.js
import express from 'express'
import { handleDemoData } from '../services/demo-data.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleDemoData(req.body)
    res.json(data)
  } catch (error) {
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/demo-data', {
        prompt: '整理 demo 数据',
...
上面的最小闭环跑通后，就要把 `整理 demo 数据` 的真实语义补进去，而不是永远停在 demo 数据。
...
这一节你不该只记住 **整理 demo 数据** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
