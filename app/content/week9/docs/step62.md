# Step 62: 封装 embedding service

## 学习目标

这一节不再写成“看起来像懂了”的纯讲义，而是直接以你已经实现过的项目 **`/Users/jianglin/Desktop/backend/AI-backend`** 为练习底座，讲清楚 **封装 embedding service** 应该怎么落进一套真实 Node 后端工程里。
...
> **本节目标：** 把 embedding / index / search 统一封装。
...
src/
├── routes/
│   └── embedding-service.routes.js          # 新增：本节练习接口
├── controllers/
│   └── embedding-service.controller.js      # 新增：请求入口
├── services/
│   └── embedding-service.service.js         # 新增：核心逻辑
...
创建文件：`src/services/embedding-service.service.js`
...
class EmbeddingServiceService {
  async run(payload = {}) {
    ...
    logs.push({ stage: 'action', content: '执行 封装 embedding service 的核心逻辑' })

    const result = {
      ok: true,
      feature: 'embedding-service',
      payload,
      summary: '封装 embedding service 的最小实现已经打通',
      completedAt: new Date().toISOString(),
    }

    logs.push({ stage: 'observation', content: result.summary })

    logger.info('embedding-service service completed', {
...
export default new EmbeddingServiceService()
```
...
创建文件：`src/controllers/embedding-service.controller.js`

```js
import { success } from '../utils/response.js'
import embeddingServiceService from '../services/embedding-service.service.js'

class EmbeddingServiceController {
  async run(req, res) {
    const data = await embeddingServiceService.run(req.body)
    return res.json(success(data, '封装 embedding service 执行成功'))
  }
}

export default new EmbeddingServiceController()
```
...
创建文件：`src/routes/embedding-service.routes.js`

```js
import express from 'express'
import embeddingServiceController from '../controllers/embedding-service.controller.js'
import { asyncHandler } from '../utils/asyncHandler.js'

const router = express.Router()

router.post('/embedding-service', asyncHandler(embeddingServiceController.run.bind(embeddingServiceController)))
...
```js
import embeddingserviceRoutes from './embedding-service.routes.js'
```

在路由挂载区新增：

```js
router.use('/', embeddingserviceRoutes)
```
...
### 4.1 围绕“封装 embedding service”的真实实现方向
...
```bash
curl -X POST http://localhost:3000/api/embedding-service   -H 'Content-Type: application/json'   -d '{
    "input": "test 封装 embedding service",
    "debug": true
  }'
```
...
{
  "success": true,
  "message": "封装 embedding service 执行成功",
  "data": {
    "result": {
      "ok": true,
      "feature": "embedding-service"
    ...
    "logs": [
      ...
      { "stage": "action", "content": "执行 封装 embedding service 的核心逻辑" },
      { "stage": "observation", "content": "封装 embedding service 的最小实现已经打通" }
    ]
  }
}
```
...
- 我知道怎样把 **封装 embedding service** 放进一套真实后端工程
