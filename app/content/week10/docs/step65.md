...
这一节不只讲概念，而是要把 **对不同 chunk 长度做实验** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 实现 chunk 对比实验，记录不同参数效果。

---

## 一、先把问题看清楚

如果只看文字解释，**对不同 chunk 长度做实验** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week10/code/chunk-experiment
```

这一节建议至少准备这些文件：

```
week10/code/chunk-experiment/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week10/code/chunk-experiment/server-demo.js
```
...
```js
// app/server/services/chunk-experiment.service.js
export async function handleChunkExperiment(payload) {
  return {
    ok: true,
    feature: 'chunk-experiment',
    payload,
    timestamp: Date.now(),
  }
}
...
```js
// app/server/routes/chunk-experiment.js
import express from 'express'
import { handleChunkExperiment } from '../services/chunk-experiment.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleChunkExperiment(req.body)
    res.json(data)
  } catch (error) {
...
```js
import chunkexperimentRouter from './routes/chunk-experiment.js'
app.use('/api/chunk-experiment', chunkexperimentRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/chunk-experiment', {
        prompt: '对不同 chunk 长度做实验',
...
上面的最小闭环跑通后，就要把 `对不同 chunk 长度做实验` 的真实语义补进去，而不是永远停在 demo 数据。
...
**围绕「对不同 chunk 长度做实验」时，建议重点做下面这些事：**
...
这一节你不该只记住 **对不同 chunk 长度做实验** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
