...
这一节不只讲概念，而是要把 **加入元数据（文档名、页码）** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 给 chunk 增加文档名、标题、段落索引等元数据。

---

## 一、先把问题看清楚

如果只看文字解释，**加入元数据（文档名、页码）** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week10/code/chunk-metadata
```

这一节建议至少准备这些文件：

```
week10/code/chunk-metadata/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week10/code/chunk-metadata/server-demo.js
```
...
```js
// app/server/services/chunk-metadata.service.js
export async function handleChunkMetadata(payload) {
  return {
    ok: true,
...
```js
// app/server/routes/chunk-metadata.js
import express from 'express'
import { handleChunkMetadata } from '../services/chunk-metadata.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleChunkMetadata(req.body)
    res.json(data)
  } catch (error) {
...
```js
import chunkmetadataRouter from './routes/chunk-metadata.js'
app.use('/api/chunk-metadata', chunkmetadataRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/chunk-metadata', {
        prompt: '加入元数据（文档名、页码）',
...
上面的最小闭环跑通后，就要把 `加入元数据（文档名、页码）` 的真实语义补进去，而不是永远停在 demo 数据。
...
这一节你不该只记住 **加入元数据（文档名、页码）** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
