...
这一节不只讲概念，而是要把 **实验 chunk 对召回率影响** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 在前端对比不同 chunk 策略下的召回结果。

---

## 一、先把问题看清楚

如果只看文字解释，**实验 chunk 对召回率影响** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week10/code/recall-test
```

这一节建议至少准备这些文件：

```
week10/code/recall-test/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week10/code/recall-test/server-demo.js
```
...
```js
// app/server/services/recall-test.service.js
export async function handleRecallTest(payload) {
  return {
    ok: true,
    feature: 'recall-test',
    payload,
...
```js
// app/server/routes/recall-test.js
import express from 'express'
import { handleRecallTest } from '../services/recall-test.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleRecallTest(req.body)
    res.json(data)
  } catch (error) {
...
```js
import recalltestRouter from './routes/recall-test.js'
app.use('/api/recall-test', recalltestRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/recall-test', {
        prompt: '实验 chunk 对召回率影响',
...
上面的最小闭环跑通后，就要把 `实验 chunk 对召回率影响` 的真实语义补进去，而不是永远停在 demo 数据。
...
**围绕「实验 chunk 对召回率影响」时，建议重点做下面这些事：**
...
这一节你不该只记住 **实验 chunk 对召回率影响** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
