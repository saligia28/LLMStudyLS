# Step 52: 实现文件 diff 输出

## 学习目标

这一节不再写成“看起来像懂了”的纯讲义，而是直接以你已经实现过的项目 **`/Users/jianglin/Desktop/backend/AI-backend`** 为练习底座，讲清楚 **实现文件 diff 输出** 应该怎么落进一套真实 Node 后端工程里。
...
src/
├── routes/
│   └── diff.routes.js          # 新增：本节练习接口
├── controllers/
│   └── diff.controller.js      # 新增：请求入口
├── services/
│   └── diff.service.js         # 新增：核心逻辑
...
创建文件：`src/services/diff.service.js`
...
class DiffService {
  async run(payload = {}) {
    ...
    logs.push({ stage: 'action', content: '执行 实现文件 diff 输出 的核心逻辑' })

    const result = {
      ok: true,
      feature: 'diff',
      payload,
      summary: '实现文件 diff 输出 的最小实现已经打通',
      completedAt: new Date().toISOString(),
    }

    logs.push({ stage: 'observation', content: result.summary })

    logger.info('diff service completed', {
...
创建文件：`src/controllers/diff.controller.js`
...
class DiffController {
  async run(req, res) {
    const data = await diffService.run(req.body)
    return res.json(success(data, '实现文件 diff 输出 执行成功'))
  }
}

export default new DiffController()
```
...
创建文件：`src/routes/diff.routes.js`
...
router.post('/diff', asyncHandler(diffController.run.bind(diffController)))
...
```bash
curl -X POST http://localhost:3000/api/diff   -H 'Content-Type: application/json'   -d '{
    "input": "test 实现文件 diff 输出",
    "debug": true
  }'
```
...
```json
{
  ...
  "data": {
    ...
    "logs": [
      ...
      { "stage": "action", "content": "执行 实现文件 diff 输出 的核心逻辑" },
      { "stage": "observation", "content": "实现文件 diff 输出 的最小实现已经打通" }
    ]
  }
}
```
...
- 我知道怎样把 **实现文件 diff 输出** 放进一套真实后端工程
