...
这一节不只讲概念，而是要把 **加入任务链（plan → execute → verify）** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 实现 plan / execute / verify 三阶段任务链。

---

## 一、先把问题看清楚

如果只看文字解释，**加入任务链（plan → execute → verify）** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week8/code/task-chain
```

这一节建议至少准备这些文件：

```
week8/code/task-chain/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
async function main() {
  const task = '加入任务链（plan → execute → verify）'
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week8/code/task-chain/server-demo.js
```
...
```js
// app/server/services/task-chain.service.js
export async function handleTaskChain(payload) {
  return {
    ok: true,
    feature: 'task-chain',
    payload,
...
```js
// app/server/routes/task-chain.js
import express from 'express'
import { handleTaskChain } from '../services/task-chain.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleTaskChain(req.body)
    res.json(data)
  } catch (error) {
...
```js
import taskchainRouter from './routes/task-chain.js'
app.use('/api/task-chain', taskchainRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/task-chain', {
        prompt: '加入任务链（plan → execute → verify）',
  ...
  return (
    <div style={{ padding: 16 }}>
      <h3>加入任务链（plan → execute → verify）</h3>
...
上面的最小闭环跑通后，就要把 `加入任务链（plan → execute → verify）` 的真实语义补进去，而不是永远停在 demo 数据。
...
**围绕「加入任务链（plan → execute → verify）」时，建议重点做下面这些事：**
...
这一节你不该只记住 **加入任务链（plan → execute → verify）** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
