...
这一节不只讲概念，而是要把 **设置权限控制** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 增加操作级权限判断，区分 read / write / exec。

---

## 一、先把问题看清楚

如果只看文字解释，**设置权限控制** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week8/code/permission-control
```

这一节建议至少准备这些文件：

```
week8/code/permission-control/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week8/code/permission-control/server-demo.js
```
...
```js
// app/server/services/permission-control.service.js
export async function handlePermissionControl(payload) {
  return {
    ok: true,
...
```js
// app/server/routes/permission-control.js
import express from 'express'
import { handlePermissionControl } from '../services/permission-control.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handlePermissionControl(req.body)
    res.json(data)
  } catch (error) {
...
```js
import permissioncontrolRouter from './routes/permission-control.js'
app.use('/api/permission-control', permissioncontrolRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/permission-control', {
        prompt: '设置权限控制',
...
上面的最小闭环跑通后，就要把 `设置权限控制` 的真实语义补进去，而不是永远停在 demo 数据。
...
这一节你不该只记住 **设置权限控制** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
