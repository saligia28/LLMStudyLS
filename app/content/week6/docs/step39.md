...
这一节不只讲概念，而是要把 **加入多工具选择逻辑** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 让 agent 在 search / read / llm 三种能力间自动选择。

---

## 一、先把问题看清楚

如果只看文字解释，**加入多工具选择逻辑** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week6/code/tool-selection
```

这一节建议至少准备这些文件：

```
week6/code/tool-selection/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week6/code/tool-selection/server-demo.js
```
...
```js
// app/server/services/tool-selection.service.js
export async function handleToolSelection(payload) {
  return {
    ok: true,
...
```js
// app/server/routes/tool-selection.js
import express from 'express'
import { handleToolSelection } from '../services/tool-selection.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleToolSelection(req.body)
    res.json(data)
  } catch (error) {
...
```js
import toolselectionRouter from './routes/tool-selection.js'
app.use('/api/tool-selection', toolselectionRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/tool-selection', {
        prompt: '加入多工具选择逻辑',
...
上面的最小闭环跑通后，就要把 `加入多工具选择逻辑` 的真实语义补进去，而不是永远停在 demo 数据。
...
这一节你不该只记住 **加入多工具选择逻辑** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
