...
这一节不再写成“看起来像懂了”的纯讲义，而是直接以你已经实现过的项目 **`/Users/jianglin/Desktop/backend/AI-backend`** 为练习底座，讲清楚 **加入多工具选择逻辑** 应该怎么落进一套真实 Node 后端工程里。
...
> **本节目标：** 让 Agent 在 search / function / direct answer 之间自动决策。
...
src/
├── routes/
│   └── tool-routing.routes.js          # 新增：本节练习接口
├── controllers/
│   └── tool-routing.controller.js      # 新增：请求入口
├── services/
│   └── tool-routing.service.js         # 新增：核心逻辑
...
创建文件：`src/services/tool-routing.service.js`
...
class ToolRoutingService {
  async run(payload = {}) {
    ...
    logs.push({ stage: 'action', content: '执行 加入多工具选择逻辑 的核心逻辑' })

    const result = {
      ok: true,
      feature: 'tool-routing',
      payload,
      summary: '加入多工具选择逻辑 的最小实现已经打通',
      completedAt: new Date().toISOString(),
    }

    logs.push({ stage: 'observation', content: result.summary })

    logger.info('tool-routing service completed', {
...
export default new ToolRoutingService()
```
...
创建文件：`src/controllers/tool-routing.controller.js`

```js
import { success } from '../utils/response.js'
import toolRoutingService from '../services/tool-routing.service.js'

class ToolRoutingController {
  async run(req, res) {
    const data = await toolRoutingService.run(req.body)
    return res.json(success(data, '加入多工具选择逻辑 执行成功'))
  }
}

export default new ToolRoutingController()
```
...
创建文件：`src/routes/tool-routing.routes.js`

```js
import express from 'express'
import toolRoutingController from '../controllers/tool-routing.controller.js'
import { asyncHandler } from '../utils/asyncHandler.js'

const router = express.Router()

router.post('/tool-routing', asyncHandler(toolRoutingController.run.bind(toolRoutingController)))
...
```js
import toolroutingRoutes from './tool-routing.routes.js'
```

在路由挂载区新增：

```js
router.use('/', toolroutingRoutes)
```
...
```bash
curl -X POST http://localhost:3000/api/tool-routing   -H 'Content-Type: application/json'   -d '{
    "input": "test 加入多工具选择逻辑",
    "debug": true
  }'
```
...
```json
{
  ...
  "data": {
    ...
    "logs": [
      ...
      { "stage": "action", "content": "执行 加入多工具选择逻辑 的核心逻辑" },
      { "stage": "observation", "content": "加入多工具选择逻辑 的最小实现已经打通" }
    ]
  }
}
```
