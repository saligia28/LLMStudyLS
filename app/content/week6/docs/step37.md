# Step 37: 实现 action → observation → next action 循环

## 学习目标

这一节不再写成“看起来像懂了”的纯讲义，而是直接以你已经实现过的项目 **`/Users/jianglin/Desktop/backend/AI-backend`** 为练习底座，讲清楚 **实现 action → observation → next action 循环** 应该怎么落进一套真实 Node 后端工程里。
...
> **本节目标：** 把单步执行升级成可循环多步推进的 agent runner。
...
围绕 **实现 action → observation → next action 循环**，建议这样放：
...
src/
├── routes/
│   └── react-loop.routes.js          # 新增：本节练习接口
├── controllers/
│   └── react-loop.controller.js      # 新增：请求入口
├── services/
│   └── react-loop.service.js         # 新增：核心逻辑
...
创建文件：`src/services/react-loop.service.js`
...
class ReactLoopService {
  async run(payload = {}) {
    ...
    logs.push({ stage: 'action', content: '执行 实现 action → observation → next action 循环 的核心逻辑' })

    const result = {
      ok: true,
      feature: 'react-loop',
      payload,
      summary: '实现 action → observation → next action 循环 的最小实现已经打通',
      completedAt: new Date().toISOString(),
    }

    logs.push({ stage: 'observation', content: result.summary })

    logger.info('react-loop service completed', {
...
export default new ReactLoopService()
```
...
创建文件：`src/controllers/react-loop.controller.js`

```js
import { success } from '../utils/response.js'
import reactLoopService from '../services/react-loop.service.js'

class ReactLoopController {
  async run(req, res) {
    const data = await reactLoopService.run(req.body)
    return res.json(success(data, '实现 action → observation → next action 循环 执行成功'))
  }
}

export default new ReactLoopController()
```
...
创建文件：`src/routes/react-loop.routes.js`

```js
import express from 'express'
import reactLoopController from '../controllers/react-loop.controller.js'
import { asyncHandler } from '../utils/asyncHandler.js'

const router = express.Router()

router.post('/react-loop', asyncHandler(reactLoopController.run.bind(reactLoopController)))
...
```js
import reactloopRoutes from './react-loop.routes.js'
```

在路由挂载区新增：

```js
router.use('/', reactloopRoutes)
```
...
### 4.1 围绕“实现 action → observation → next action 循环”的真实实现方向
...
```bash
curl -X POST http://localhost:3000/api/react-loop   -H 'Content-Type: application/json'   -d '{
    "input": "test 实现 action → observation → next action 循环",
    "debug": true
  }'
```
...
{
  "success": true,
  "message": "实现 action → observation → next action 循环 执行成功",
  "data": {
    "result": {
      "ok": true,
      "feature": "react-loop"
    ...
    "logs": [
      ...
      { "stage": "action", "content": "执行 实现 action → observation → next action 循环 的核心逻辑" },
      { "stage": "observation", "content": "实现 action → observation → next action 循环 的最小实现已经打通" }
    ]
  }
}
```
...
- 我知道怎样把 **实现 action → observation → next action 循环** 放进一套真实后端工程
