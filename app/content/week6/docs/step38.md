# Step 38: 构建一个能自动搜索 + 再处理的 Agent

## 学习目标

这一节不再写成“看起来像懂了”的纯讲义，而是直接以你已经实现过的项目 **`/Users/jianglin/Desktop/backend/AI-backend`** 为练习底座，讲清楚 **构建一个能自动搜索 + 再处理的 Agent** 应该怎么落进一套真实 Node 后端工程里。
...
> **本节目标：** 增加 search 工具，让 Agent 先查资料再总结。
...
src/
├── routes/
│   └── search-agent.routes.js          # 新增：本节练习接口
├── controllers/
│   └── search-agent.controller.js      # 新增：请求入口
├── services/
│   └── search-agent.service.js         # 新增：核心逻辑
...
创建文件：`src/services/search-agent.service.js`
...
class SearchAgentService {
  async run(payload = {}) {
    ...
    logs.push({ stage: 'action', content: '执行 构建一个能自动搜索 + 再处理的 Agent 的核心逻辑' })

    const result = {
      ok: true,
      feature: 'search-agent',
      payload,
      summary: '构建一个能自动搜索 + 再处理的 Agent 的最小实现已经打通',
      completedAt: new Date().toISOString(),
    }

    logs.push({ stage: 'observation', content: result.summary })

    logger.info('search-agent service completed', {
...
export default new SearchAgentService()
```
...
创建文件：`src/controllers/search-agent.controller.js`

```js
import { success } from '../utils/response.js'
import searchAgentService from '../services/search-agent.service.js'

class SearchAgentController {
  async run(req, res) {
    const data = await searchAgentService.run(req.body)
    return res.json(success(data, '构建一个能自动搜索 + 再处理的 Agent 执行成功'))
  }
}

export default new SearchAgentController()
```
...
创建文件：`src/routes/search-agent.routes.js`

```js
import express from 'express'
import searchAgentController from '../controllers/search-agent.controller.js'
import { asyncHandler } from '../utils/asyncHandler.js'

const router = express.Router()

router.post('/search-agent', asyncHandler(searchAgentController.run.bind(searchAgentController)))
...
```js
import searchagentRoutes from './search-agent.routes.js'
```

在路由挂载区新增：

```js
router.use('/', searchagentRoutes)
```
...
### 4.1 围绕“构建一个能自动搜索 + 再处理的 Agent”的真实实现方向
...
```bash
curl -X POST http://localhost:3000/api/search-agent   -H 'Content-Type: application/json'   -d '{
    "input": "test 构建一个能自动搜索 + 再处理的 Agent",
    "debug": true
  }'
```
...
{
  "success": true,
  "message": "构建一个能自动搜索 + 再处理的 Agent 执行成功",
  "data": {
    "result": {
      "ok": true,
      "feature": "search-agent"
    ...
    "logs": [
      ...
      { "stage": "action", "content": "执行 构建一个能自动搜索 + 再处理的 Agent 的核心逻辑" },
      { "stage": "observation", "content": "构建一个能自动搜索 + 再处理的 Agent 的最小实现已经打通" }
    ]
  }
}
```
...
- 我知道怎样把 **构建一个能自动搜索 + 再处理的 Agent** 放进一套真实后端工程
