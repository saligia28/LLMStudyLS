...
这一节不只讲概念，而是要把 **构建一个能自动搜索 + 再处理的 Agent** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 加入 search 工具，支持先检索本地内容再汇总回答。

---

## 一、先把问题看清楚

如果只看文字解释，**构建一个能自动搜索 + 再处理的 Agent** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week6/code/search-agent
```

这一节建议至少准备这些文件：

```
week6/code/search-agent/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
async function main() {
  const task = '构建一个能自动搜索 + 再处理的 Agent'
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week6/code/search-agent/server-demo.js
```
...
```js
// app/server/services/search-agent.service.js
export async function handleSearchAgent(payload) {
  return {
    ok: true,
...
```js
// app/server/routes/search-agent.js
import express from 'express'
import { handleSearchAgent } from '../services/search-agent.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleSearchAgent(req.body)
    res.json(data)
  } catch (error) {
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/search-agent', {
        prompt: '构建一个能自动搜索 + 再处理的 Agent',
...
上面的最小闭环跑通后，就要把 `构建一个能自动搜索 + 再处理的 Agent` 的真实语义补进去，而不是永远停在 demo 数据。
...
**围绕「构建一个能自动搜索 + 再处理的 Agent」时，建议重点做下面这些事：**
...
这一节你不该只记住 **构建一个能自动搜索 + 再处理的 Agent** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
