...
这一节不只讲概念，而是要把 **测试复杂任务（翻译、问答、推理）** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 为 agent 增加任务测试面板，观察不同任务类型表现。

---

## 一、先把问题看清楚

如果只看文字解释，**测试复杂任务（翻译、问答、推理）** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week6/code/task-bench
```

这一节建议至少准备这些文件：

```
week6/code/task-bench/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week6/code/task-bench/server-demo.js
```
...
```js
// app/server/services/task-bench.service.js
export async function handleTaskBench(payload) {
  return {
    ok: true,
    feature: 'task-bench',
    payload,
...
```js
// app/server/routes/task-bench.js
import express from 'express'
import { handleTaskBench } from '../services/task-bench.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleTaskBench(req.body)
    res.json(data)
  } catch (error) {
...
```js
import taskbenchRouter from './routes/task-bench.js'
app.use('/api/task-bench', taskbenchRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/task-bench', {
        prompt: '测试复杂任务（翻译、问答、推理）',
...
上面的最小闭环跑通后，就要把 `测试复杂任务（翻译、问答、推理）` 的真实语义补进去，而不是永远停在 demo 数据。
...
**围绕「测试复杂任务（翻译、问答、推理）」时，建议重点做下面这些事：**
...
这一节你不该只记住 **测试复杂任务（翻译、问答、推理）** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
