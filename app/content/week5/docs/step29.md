...
// 定义工具列表（新版 API 使用 tools，不再使用 functions）
const tools = [
  {
    type: 'function',          // 固定值
    function: {
...
const response = await client.chat.completions.create({
  ...
  tools: tools,          // 新版：使用 tools 字段
  tool_choice: 'auto',  // 'auto' | 'none' | 特定函数
})
...
```javascript
{
  ...
  tool_calls: [   // 新版：tool_calls 数组（可同时调用多个函数）
    {
      id: 'call_abc123',        // 每次调用的唯一 ID（返回结果时必须带上）
      type: 'function',
      function: {
...
- `tool_calls` 是**数组**,AI 可以在一次响应中请求调用多个函数
- `tool_calls[0].function.arguments` 是 **JSON 字符串**,不是对象,需要 `JSON.parse()` 解析
- `tool_calls[0].id` 是本次调用的唯一标识,将函数结果返回 AI 时必须带上
...
export class BaseAdapter {
  ...
  formatResponse(response) {
    return {
      ...
      tool_calls: response.choices[0].message.tool_calls,  // 新版字段
    }
  }
}
```
...
3. **Step 32**: 调试 tool_calls → arguments 的 JSON 化
