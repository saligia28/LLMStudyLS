...
async chat(messages, options = {}) {
  ...
  // 传递 tools 给 Adapter（新版 API 使用 tools）
  const adapter = factory.get(provider)
  const result = await adapter.chat(messages, {
    ...restOptions,
    tools: tools,  // ← 这里传入 tools（包含 Function Schemas）
  })
...
async chat(messages, options = {}) {
  ...
  const response = await this.client.chat.completions.create({
    ...
    tools: tools,                             // ← 传给 DeepSeek API（新版字段）
    tool_choice: tools ? 'auto' : undefined,  // 'auto' | 'none'
    ...restOptions,
  })
...
2. **Step 32**: 调试 tool_calls → arguments 的 JSON 化
