# Step 47: 给 Agent 加上 MCP 工具注册

## 学习目标

这一节不再写成“看起来像懂了”的纯讲义，而是直接以你已经实现过的项目 **`/Users/jianglin/Desktop/backend/AI-backend`** 为练习底座，讲清楚 **给 Agent 加上 MCP 工具注册** 应该怎么落进一套真实 Node 后端工程里。
...
src/
├── routes/
│   └── mcp-registry.routes.js          # 新增：本节练习接口
├── controllers/
│   └── mcp-registry.controller.js      # 新增：请求入口
├── services/
│   └── mcp-registry.service.js         # 新增：核心逻辑
...
创建文件：`src/services/mcp-registry.service.js`
...
class McpRegistryService {
  async run(payload = {}) {
    ...
    logs.push({ stage: 'action', content: '执行 给 Agent 加上 MCP 工具注册 的核心逻辑' })

    const result = {
      ok: true,
      feature: 'mcp-registry',
      payload,
      summary: '给 Agent 加上 MCP 工具注册 的最小实现已经打通',
      completedAt: new Date().toISOString(),
    }

    logs.push({ stage: 'observation', content: result.summary })

    logger.info('mcp-registry service completed', {
...
创建文件：`src/controllers/mcp-registry.controller.js`
...
class McpRegistryController {
  async run(req, res) {
    const data = await mcpRegistryService.run(req.body)
    return res.json(success(data, '给 Agent 加上 MCP 工具注册 执行成功'))
  }
}

export default new McpRegistryController()
```
...
创建文件：`src/routes/mcp-registry.routes.js`

```js
import express from 'express'
import mcpRegistryController from '../controllers/mcp-registry.controller.js'
import { asyncHandler } from '../utils/asyncHandler.js'

const router = express.Router()

router.post('/mcp-registry', asyncHandler(mcpRegistryController.run.bind(mcpRegistryController)))
...
### 4.1 围绕“给 Agent 加上 MCP 工具注册”的真实实现方向
...
```bash
curl -X POST http://localhost:3000/api/mcp-registry   -H 'Content-Type: application/json'   -d '{
    "input": "test 给 Agent 加上 MCP 工具注册",
    "debug": true
  }'
```
...
{
  "success": true,
  "message": "给 Agent 加上 MCP 工具注册 执行成功",
  "data": {
    "result": {
      "ok": true,
      "feature": "mcp-registry"
    ...
    "logs": [
      ...
      { "stage": "action", "content": "执行 给 Agent 加上 MCP 工具注册 的核心逻辑" },
      { "stage": "observation", "content": "给 Agent 加上 MCP 工具注册 的最小实现已经打通" }
    ]
  }
}
```
...
- 我知道怎样把 **给 Agent 加上 MCP 工具注册** 放进一套真实后端工程
