...
这一节不只讲概念，而是要把 **给 Agent 加上 MCP 工具注册** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 把 MCP 注册表接进 agent，让模型能发现工具。

---

## 一、先把问题看清楚

如果只看文字解释，**给 Agent 加上 MCP 工具注册** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week7/code/mcp-register
```

这一节建议至少准备这些文件：

```
week7/code/mcp-register/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
async function main() {
  const task = '给 Agent 加上 MCP 工具注册'
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week7/code/mcp-register/server-demo.js
```
...
```js
// app/server/services/mcp-register.service.js
export async function handleMcpRegister(payload) {
  return {
    ok: true,
...
```js
// app/server/routes/mcp-register.js
import express from 'express'
import { handleMcpRegister } from '../services/mcp-register.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleMcpRegister(req.body)
    res.json(data)
  } catch (error) {
...
```js
import mcpregisterRouter from './routes/mcp-register.js'
app.use('/api/mcp-register', mcpregisterRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/mcp-register', {
        prompt: '给 Agent 加上 MCP 工具注册',
...
上面的最小闭环跑通后，就要把 `给 Agent 加上 MCP 工具注册` 的真实语义补进去，而不是永远停在 demo 数据。
...
**围绕「给 Agent 加上 MCP 工具注册」时，建议重点做下面这些事：**
...
这一节你不该只记住 **给 Agent 加上 MCP 工具注册** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
