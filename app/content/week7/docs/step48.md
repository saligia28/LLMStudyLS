# Step 48: 测试：让 AI 自动修改项目代码

## 学习目标

这一节不再写成“看起来像懂了”的纯讲义，而是直接以你已经实现过的项目 **`/Users/jianglin/Desktop/backend/AI-backend`** 为练习底座，讲清楚 **测试：让 AI 自动修改项目代码** 应该怎么落进一套真实 Node 后端工程里。
...
src/
├── routes/
│   └── agent-edit-code.routes.js          # 新增：本节练习接口
├── controllers/
│   └── agent-edit-code.controller.js      # 新增：请求入口
├── services/
│   └── agent-edit-code.service.js         # 新增：核心逻辑
...
创建文件：`src/services/agent-edit-code.service.js`

```js
import logger from '../utils/logger.js'

class AgentEditCodeService {
  async run(payload = {}) {
    const startTime = Date.now()
    const logs = []

    logs.push({ stage: 'thought', content: '开始分析任务' })
    logs.push({ stage: 'action', content: '执行 测试：让 AI 自动修改项目代码 的核心逻辑' })

    const result = {
      ok: true,
      feature: 'agent-edit-code',
      payload,
      summary: '测试：让 AI 自动修改项目代码 的最小实现已经打通',
      completedAt: new Date().toISOString(),
    }

    logs.push({ stage: 'observation', content: result.summary })

    logger.info('agent-edit-code service completed', {
...
export default new AgentEditCodeService()
```
...
创建文件：`src/controllers/agent-edit-code.controller.js`

```js
import { success } from '../utils/response.js'
import agentEditCodeService from '../services/agent-edit-code.service.js'

class AgentEditCodeController {
  async run(req, res) {
    const data = await agentEditCodeService.run(req.body)
    return res.json(success(data, '测试：让 AI 自动修改项目代码 执行成功'))
  }
}

export default new AgentEditCodeController()
```
...
创建文件：`src/routes/agent-edit-code.routes.js`

```js
import express from 'express'
import agentEditCodeController from '../controllers/agent-edit-code.controller.js'
import { asyncHandler } from '../utils/asyncHandler.js'

const router = express.Router()

router.post('/agent-edit-code', asyncHandler(agentEditCodeController.run.bind(agentEditCodeController)))
...
```js
import agenteditcodeRoutes from './agent-edit-code.routes.js'
```

在路由挂载区新增：

```js
router.use('/', agenteditcodeRoutes)
```
...
### 4.1 围绕“测试：让 AI 自动修改项目代码”的真实实现方向
...
```bash
curl -X POST http://localhost:3000/api/agent-edit-code   -H 'Content-Type: application/json'   -d '{
    "input": "test 测试：让 AI 自动修改项目代码",
    "debug": true
  }'
```
...
```json
{
  ...
  "data": {
    ...
    "logs": [
      ...
      { "stage": "action", "content": "执行 测试：让 AI 自动修改项目代码 的核心逻辑" },
      { "stage": "observation", "content": "测试：让 AI 自动修改项目代码 的最小实现已经打通" }
    ]
  }
}
```
...
- 我知道怎样把 **测试：让 AI 自动修改项目代码** 放进一套真实后端工程
