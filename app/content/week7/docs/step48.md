...
这一节不只讲概念，而是要把 **测试：让 AI 自动修改项目代码** 真的做出来。做完后，你应该能在当前项目里亲手跑通一套最小可工作的 Node + 前端联动示例，而不是停留在“看懂了原理”的温柔幻觉里。
...
> **本节目标效果：** 做一个受控修改 demo，让 agent 修改 demo 文件内容。

---

## 一、先把问题看清楚

如果只看文字解释，**测试：让 AI 自动修改项目代码** 很容易被误解成一个抽象概念；但对前端开发来说，真正有价值的是：
...
```bash
mkdir -p /Users/jianglin/Desktop/LLMStudyLS/app/content/week7/code/code-edit-test
```

这一节建议至少准备这些文件：

```
week7/code/code-edit-test/
├── README.md
├── server-demo.js
├── service.js
└── client-demo.jsx
...
```bash
node /Users/jianglin/Desktop/LLMStudyLS/app/content/week7/code/code-edit-test/server-demo.js
```
...
```js
// app/server/services/code-edit-test.service.js
export async function handleCodeEditTest(payload) {
  return {
    ok: true,
    feature: 'code-edit-test',
    payload,
    timestamp: Date.now(),
  }
}
...
```js
// app/server/routes/code-edit-test.js
import express from 'express'
import { handleCodeEditTest } from '../services/code-edit-test.service.js'

const router = express.Router()

router.post('/', async (req, res) => {
  try {
    const data = await handleCodeEditTest(req.body)
    res.json(data)
  } catch (error) {
...
```js
import codeedittestRouter from './routes/code-edit-test.js'
app.use('/api/code-edit-test', codeedittestRouter)
```
...
export default function ClientDemo() {
  ...
  async function run() {
    setLoading(true)
    try {
      const res = await axios.post('http://localhost:3001/api/code-edit-test', {
        prompt: '测试：让 AI 自动修改项目代码',
...
上面的最小闭环跑通后，就要把 `测试：让 AI 自动修改项目代码` 的真实语义补进去，而不是永远停在 demo 数据。
...
**围绕「测试：让 AI 自动修改项目代码」时，建议重点做下面这些事：**
...
这一节你不该只记住 **测试：让 AI 自动修改项目代码** 的定义，而是应该真的把它做成一个前端能点、后端能跑、日志能看的最小系统。
